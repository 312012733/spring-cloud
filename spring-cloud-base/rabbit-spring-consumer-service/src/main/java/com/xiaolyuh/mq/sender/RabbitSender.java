...
public class RabbitSender implements RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnCallback, InitializingBean
{
    ...
    /**
     * 用于实现消息发送到RabbitMQ交换器后接收ack回调。 如果消息发送确认失败就进行重试。
     *
    ...
    /**
     * 用于实现消息发送到RabbitMQ交换器，但无相应队列与交换器绑定时的回调。 在脑裂的情况下会出现这种情况
     */
    @Override
    public void returnedMessage(Message message, int replyCode, String replyText, String exchange, String routingKey)
    {
        logger.error("MQ消息发送失败，replyCode:{}, replyText:{}，exchange:{}，routingKey:{}，消息体:{}", replyCode, replyText,
                exchange, routingKey, JSON.toJSONString(message.getBody()));
